"""Demo wibbled edge generation.

This script runs wibbled edge generation and plots samples of curvature vectors,
noise vectors, flattening function, and wibbled paths. See edge_wibble.py for
details of what these all mean.
"""

import numpy as np
from matplotlib import pyplot as plt
import edge_wibble as edge_wibble
import time
...
def main():
    ...
    for _ in range(100):
        wibbled_path_object(start=np.array([0., 0.]), end=np.array([1., 1.]))
    path_end_time = time.time()
    time_per_path = (path_end_time - path_start_time) / 100
    print(f'Wibbled path runtime: {time_per_path}')
