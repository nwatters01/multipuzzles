...
This file contains code for wibbling edges of the puzzle. Wibbling edges
introduces folds and ox-bows that make the pieces fit together in a more
satisfying way.

Each edge of the puzzle is wibbled independently, subject only to rejection if
one edge intersects another after wibbling. The wibbling of an edge is done with
the following steps:
...
def get_sample_curve_length(min_integral=1.2 * np.pi,
...
def get_sample_straight_length(min_length=0.03, max_length=0.05):
...
class WibbledPath():
    ...
    This class generated a wibbled path when called. Generating a wibbled path
    takes these steps:
    ...
    def __init__(self,
        ...
        Args:
            ...
            path_squishing_factor: Float no greater than 1. Multiplier to
                ...
                for avoiding intersections between nearby wibbled edges in the
                puzzle.
        ...
        if self._total_length > self._resolution:
            raise ValueError(
                f'total_length {self._total_length} must be less than or equal '
        ...
        linspace = np.linspace(
            0., 1., self._total_length // self._self_intersecting_stride)
    ...
    def __call__(self, start, end):
        # Sample and integrate curvature to get a wibbled path
...
class WibbledEdges():
    ...
    This is a transformation that wibbles the edges of a puzzle when called on
    it. Only the edges of the puzzle that are not along holes are wibbled.
    Wibbling an edge consists of replacing the edge with a wibbled path
    generated from a WibbledPath() instance, subject to rejection sampling if
    nearby puzzle edges intersect.
    """

    def __init__(self,  wibbled_path_object, intersection_thresh=0.04):
        """Constructor.
        
        Args:
            wibbled_path_object: Instance of WibbledPath.
            intersection_thresh: Float. Threshold distance. If two wibbled edges
                come within this distance of each other, they are considered
                intersecting and one of them is resampled.
        """
        logging.info('Constructing WibbledEdges transform')

        self._wibbled_path_object = wibbled_path_object
    ...
    def __call__(self, puzzle):
        ...
        logging.info('    Applying edge wibbling')

        start_time = time.time()
        mesh = puzzle.mesh

        # Identify which edges are parts of holes so we don't wibble them
        all_hole_edges = []
        for hole in puzzle.holes:
            all_hole_edges.extend(hole['edges'])

        # Wibble all the non-hole edges
        wibbled_edges = []
        for k, v in mesh.items():
            ...
            for edge in puzzle.edges:
                if k[0] in edge or k[1] in edge:
                    if edge == k:
                        continue
                    if edge in wibbled_edges:
                        neighboring_edges.append(edge)
            
            # Sample a wibbled edge that does not intersect neighboring edges
            # using rejection sampling.
            success = False
            for _ in range(_MAX_TRIES):
                wibbled_edge = self._wibbled_path_object(start, end)
                intersects = False
                for edge in neighboring_edges:
                    other_edge_mesh = mesh[edge]
                    if self._intersects(wibbled_edge, other_edge_mesh):
                        intersects = True
                        break
                if intersects:
                    continue
            ...
            if not success:
                raise ValueError(
                    ...
                    'probably trying to wibble edges too aggressively and '
                    'should adjust parameters to make edges more flat.'
                )

            wibbled_edges.append(k)
            mesh[k] = wibbled_edge
        
        # Report runtime of this function
        end_time = time.time()
        logging.info(f'    Edge wibbling runtime: {end_time - start_time}')
